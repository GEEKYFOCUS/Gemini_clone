...
import "../index.css";
import styles from "./GenerativeContent.module.css";
...
function GenerativeContent() {
  ...
  const { initialContent } = useText(); // Fixed typo from 'initailContent'
  console.log("Initial Content:", initialContent, "Previous Content:", previousContent);
  const { generatedMessage, isLoading } = useAI();

  useEffect(() => {
    if (generatedMessage && !isLoading) {
      dispatch({
        type: "add-feedback",
        payload: { userMessage: currentText, feedback: generatedMessage },
      });
    }
  }, [generatedMessage, isLoading, currentText, dispatch]);

  return (
    <div className={styles.generativeContent}>
      <div className="space-y-4"> {/* Stack content vertically */}
        <div className="flex items-center space-x-2"> {/* SVG and label on their own row */}
            ...
            >
              ...
              <defs>
                ...
                >
                  ...
                  <stop offset="0.342551" stopColor="#5f91e9c1" />
